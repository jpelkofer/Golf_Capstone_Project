...
player_adj_dk_gained_sim <- function(df, n = 5, easy = 2, medium = 9, hard = 7, holes_min = 360, hole_min_penalty = .35) {
  ...
  # Easy Hole Scores
  easy_rd_1_sims <- get_player_sims(sims = n, easy_scores, easy)
  easy_rd_2_sims <- get_player_sims(sims = n, easy_scores, easy)
  easy_rd_3_sims <- get_player_sims(sims = n, easy_scores, easy)
  easy_rd_4_sims <- get_player_sims(sims = n, easy_scores, easy)
  # Medium Hole Scores
  medium_rd_1_sims <- get_player_sims(sims = n, medium_scores, medium)
  medium_rd_2_sims <- get_player_sims(sims = n, medium_scores, medium)
  medium_rd_3_sims <- get_player_sims(sims = n, medium_scores, medium)
  medium_rd_4_sims <- get_player_sims(sims = n, medium_scores, medium)
  # Hard Hole Scores
  hard_rd_1_sims <- get_player_sims(sims = n, hard_scores, hard)
  hard_rd_2_sims <- get_player_sims(sims = n, hard_scores, hard)
  hard_rd_3_sims <- get_player_sims(sims = n, hard_scores, hard)
  hard_rd_4_sims <- get_player_sims(sims = n, hard_scores, hard)
  
  # Calculate rds1 and rds2 dk points gained
  rds_1_and_2_tbl <-
    bind_rows(easy_rd_1_sims, medium_rd_1_sims, hard_rd_1_sims,
              easy_rd_2_sims, medium_rd_2_sims, hard_rd_2_sims) %>%
    group_by(sim) %>%
    summarise(dk_pts_gained_rds_1_2 = sum(score_type))
  
  # Calculate rds3 and rds4 dk points gained
  rds_3_and_4_tbl <-
    bind_rows(easy_rd_3_sims, medium_rd_3_sims, hard_rd_3_sims,
              easy_rd_4_sims, medium_rd_3_sims, hard_rd_4_sims) %>%
    group_by(sim) %>%
